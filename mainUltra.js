...
// Hàm quản lý pool process siêu tối ưu với concurrency cao
async function runProcessPoolUltra(emails, dobList, concurrency) {
  let completed = 0;
  ...
  // Control object để worker gọi giảm concurrency
  const concurrencyControl = {
    decrease: () => {
      if (currentConcurrency > 1) {
        currentConcurrency = Math.max(1, Math.floor(currentConcurrency * 0.7));
        if (mainWindow) mainWindow.webContents.send('log-update', `[SYSTEM] Đã giảm concurrency xuống ${currentConcurrency}`);
      }
    },
  ...
  return new Promise((resolve) => {
    function launchNext() {
      while (active < currentConcurrency && queue.length > 0) {
        ...
        const worker = fork(path.join(__dirname, 'bruteWorkerUltra.js'));
        ...
        mainWindow.webContents.send('progress-update', {
          ...
          total,
          email,
          rate: Math.round((completed + 1) / ((Date.now() - startTime) / 1000) * 60)
        });
        ...
        worker.send({ type: 'start', email, dobList, concurrencyControl });
      }
    }
    launchNext();
  });
...
ipcMain.handle('start-brute-force', async (event, emails, selectedRanges, concurrency) => {
  ...
  // Tự động tính toán concurrency tối ưu cho tốc độ cao
  const optimalConcurrency = concurrency || Math.min(
    Math.max(20, Math.floor(os.cpus().length * 4)), // Ít nhất 20, tối đa CPU cores * 4
    100 // Giới hạn tối đa 100 workers
  );

  await runProcessPoolUltra(emails, dobList, optimalConcurrency);
  return true;
});
...
ipcMain.handle('get-cpu-count', () => {
  ...
  return {
    cpuCount,
    recommendedConcurrency: Math.min(Math.max(20, cpuCount * 4), 100)
  };
}); 